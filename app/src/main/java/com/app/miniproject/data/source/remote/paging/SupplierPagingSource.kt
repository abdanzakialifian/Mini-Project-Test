...
@Singleton
class SupplierPagingSource @Inject constructor(private val apiService: ApiService) :
    PagingSource<Int, SupplierResponse>() {

    private var token = ""

    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, SupplierResponse> {
        val position = params.key ?: INITIAL_POSITION

        return try {
            val response = apiService.getSuppliersList(position, params.loadSize, token)
    ...
    override fun getRefreshKey(state: PagingState<Int, SupplierResponse>): Int? =
